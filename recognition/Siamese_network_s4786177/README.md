...
![alt text](melnoma_examples.png)
Reference: (Sample images screenshot taken from (nadereafshar, 2024))
...
- Visualization: As illustrated in the diagram, maximizing J finds the point on the ROC curve (red line) that is farthest from the random chance diagonal (dashed line).
- Result: This calculation selects the single optimal threshold that achieves the maximum combined benefit of Sensitivity (detecting the disease) and Specificity (avoiding false alarms). This strategy ensures the model's output is optimized for clinical safety rather than just total correctness.


## Training, evaluation and saved outputs

Running `train.py` will train the model and save the best checkpoint (by validation AUC) to `siamese_ce.pt` (or the `save_path` you pass). The script also writes per-epoch plots to `reports/` (loss, accuracy, AUC). Example outputs generated by the code include:
...
### Loss over epochs
![alt text](<reports/figures/ce_loss final.png>)
...
### Accuracy and ROC on test
![alt text](<reports/figures/test_roc final.png>)
- The Receiver Operating Characteristic (ROC) curve is a diagnostic tool that plots the True Positive Rate on the Y-axis against the False Positive Rate on the X-axis. The Area Under the Curve (AUC) is the single metric that summarizes the model's ability to discriminate between classes.
- An AUC of 0.90 (as shown in the plot) confirms that the model has an excellent probability (90%) of correctly ranking a positive sample higher than a negative sample. The curve's steep rise toward the top-left corner demonstrates a robust ability to achieve high sensitivity with low false alarm rates.

- The Test AUC of 0.903 is the strongest indicator of performance, validating that the Siamese architecture and joint loss function created highly separable feature embeddings. The final Test Accuracy at argmax was 86.9%, which reflects the overall correctness of the predictions.
...
All results, plots, and metrics found in this README can be generated by executing the training and prediction scripts. A directory named `reports/` will be created to store all generated plots (loss, AUC, ROC, Confusion Matrices, and t-SNE).

### Downloading the Dataset
The project uses the "ISIC 2020 JPG 224x224 RESIZED" dataset. Download the ZIP file from the Kaggle link provided below and arrange the contents into the correct directory structure so the dataset.py scripts can locate the images and metadata.
Download Source: https://www.kaggle.com/datasets/nischaydnk/isic-2020-jpg-224x224-resized/data

1) Create a new dir in the PatternAnalysis-2025 folder and cd into it
```bash
mkdir data
```
2) Unzip archive folder (downloaded from link above) and its contents shold live inside the data folder
3) Verify Structure: Ensure the data folder contains the metadata CSV and the image directory inside it. The dataset.py script will successfully find the data if the structure matches this required layout:
```
<repo_root>/
├── data/               <-- Code will search upward and find this folder.
│   ├── train-metadata.csv
│   └── train-image/
│       └── image/      <-- All 224x224 JPG files are located here.
└── recognition/
    └── Siamese_network_s4786177/  <-- Where train.py/dataset.py run from.
```

### Option A: Run Full Experiment Driver Script

Execute the single driver script to run the entire pipeline (training for 10 epochs-which is changeable, and final evaluation) automatically:
...
# STEP 1: Train the model and save the best checkpoint (siamese_ce.pt)
python train.py --epochs 10 --batch_size 32

# STEP 2: Evaluate the saved checkpoint on the test set and generate plots
python predict.py
```
